...
export class GamesService {
  ...
  async findAllAdmin(
      ...
      .addSelect(
        (subQuery) =>
          subQuery
            ...
            .andWhere("gr.reactionType = 'like'"),
        'likeCount',
      )
      // Add subquery to count dislikes
      .addSelect(
        (subQuery) =>
          subQuery
            .select('COUNT(*)', 'count')
            .from('game_reactions', 'gr')
            .where('gr.gameId = game.id')
            .andWhere("gr.reactionType = 'dislike'"),
        'dislikeCount',
      );
    ...
    const sortField = validSortFields.includes(sortBy)
      ? sortBy === 'likeCount' || sortBy === 'dislikeCount'
        ? sortBy
        : `game.${sortBy}`
      : 'game.createdAt';

    query.orderBy(sortField, sortOrder).skip(skip).take(limit);

    // Use getRawAndEntities to get both entities and raw data (including subquery results)
    const [rawAndEntities, total] = await Promise.all([
      query.getRawAndEntities(),
      query.getCount(),
    ]);

    // Map the raw counts to the entities
    const data = rawAndEntities.entities.map((game, index) => {
      const raw = rawAndEntities.raw[index];
      // Attach the counts as properties (they won't be saved, just returned in response)
      (game as any).likeCount = parseInt(raw.likeCount) || 0;
      (game as any).dislikeCount = parseInt(raw.dislikeCount) || 0;
      return game;
    });
  ...
  async getGameReactionStats(
    gameId: string,
    userId?: string,
  ): Promise<GameReactionStatsDto> {
    // Verify game exists
    await this.findOne(gameId);
  ...
  async incrementPlayCount(gameId: string): Promise<void> {
    ...
    await this.gamesRepository.increment({ id: gameId }, 'playCount', 1);
  }
}
